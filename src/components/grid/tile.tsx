...
export function GridTileImage({
    ...
    return (
        <div
            className={clsx(
                "relative flex h-full w-full items-center justify-center overflow-hidden",
                {
                    ...
                    "bg-violetDark dark:bg-violetDark":
        ...
        >
            ...
            {props.src ? (
                <Image
                    className={`relative h-full w-full object-contain transition 
                        duration-300 ease-in-out hover:scale-105":${isInteractive}`}
                    {...props}
                    alt={props.title || ""}
                />
            ) : null}
            {labels ? (
                <div className='absolute left-0 bottom-0 w-3/4 text-black dark:text-white'>
                    <h3
